...
parser.add_argument("--prompt_engineer", help="use prompt engineer model?", action='store_true', default=False)
...
def main():
    print('args:', args)
    song_title, artist, summarizer, prompt_engineer = process_args(args)
    song = get_lyrics(song_title, artist)

    if song:
        lyrics = process_lyrics(song.lyrics)
        line = extract_lyric(lyrics, summarizer)
        prompt = generate_prompt(prompt_engineer, line, song_title, artist)
        img = generate_image(prompt)

        if img:
            annotated_img = annotate(img, line.lower())
            save_fig(annotated_img, song_title, artist, summarizer, prompt_engineer)
        else:
